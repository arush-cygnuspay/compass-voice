...
class TurnEngineAddItemQuantityRegressionTests(unittest.TestCase):
    def test_variant_item_still_adds_successfully_after_explicit_quantity_prompt(self):
        ...
        first = engine.process_turn(session=session, user_text="pickup")
        self.assertEqual(first.response_key, "order_type_captured_pickup")

        second = _turn(
            engine,
            session,
            "I will take a small Coke.",
            intent=Intent.ADD_ITEM,
            slots=(
                SlotValue(name="VARIANT", value="small"),
