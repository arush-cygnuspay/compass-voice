#!/usr/bin/env python3
# tools/replay_phase4_add_item_planner.py
"""Phase 4 Add-Item Planner Offline Replay + Evaluation Harness CLI.

Replays captured turn logs or fixture turns through the Phase 4 add-item
planner decision path and compares behavior across GPT modes.
...
python tools/replay_phase4_add_item_planner.py \\
    ...
    --output reports/phase4_replay_shadow.jsonl \\
    --mode shadow \\
    --max-turns 500 \\
    --filter-state idle
...
python tools/replay_phase4_add_item_planner.py \\
    --fixtures-only \\
    --mode inline \\
    --output reports/phase4_fixture_replay.jsonl
...
python tools/replay_phase4_add_item_planner.py \\
    ...
    --use-live-gpt \\
    --max-turns 50 \\
    --output reports/phase4_live_replay.jsonl

Output
------
  <output>.jsonl       — One PlannerReplayResult JSON object per line
  <output>.summary.md  — Markdown summary alongside the JSONL
"""
...
def _resolve_output_path(output_arg: str | None, mode: str) -> Path:
    ...
    return reports_dir / f"phase4_replay_{mode}.jsonl"


def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description="Phase 4 Add-Item Planner Offline Replay Harness",
    ...
    parser.add_argument(
        ...
        help="Path for output replay JSONL report. "
             "Defaults to reports/phase4_replay_<mode>.jsonl.",
    )
    parser.add_argument(
        "--mode",
    ...
    parser.add_argument(
        ...
        help="Only replay turns with this state_before value "
             "(default: idle). Pass '' to replay all states.",
    )
    parser.add_argument(
        "--filter-response-key",
        metavar="KEY",
    ...
    harness = Phase4AddItemPlannerReplayHarness(config=cfg, use_live_gpt=args.use_live_gpt)
    summary = PlannerReplaySummaryBuilder(mode=mode)
        ...
        for result in results_iter:
            ...
            if args.verbose:
                print(
                    ...
                    f"decision={result.decision or '-':15} "
