# app/nlu/matching/scorer.py
"""Scoring and ranking helpers for item-name matching.

All inputs must be pre-normalized upstream; no text cleanup happens here.
``score_item_normalized`` is the hot-path scorer.  ``score_item`` is a
backward-compatible wrapper that normalizes inline.
"""
...
def score_item_normalized(user_text: str, item_name: str) -> float:
    """Deterministic similarity score between already-normalized strings.
...
def score_item(user_text: str, item_name: str) -> float:
    """Backward-compatible wrapper.  Prefer score_item_normalized in hot-path code."""
