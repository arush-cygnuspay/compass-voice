# app/nlu/matching/quantity_parser.py
"""Quantity extraction and parsing for voice ordering.

Handles integer counts, compound number words, weight quantities, and
incremental/vague quantity signals.
"""
...
def _parse_compound_number_word(text: str) -> int | None:
    """Parse English compound number words up to 999 (e.g. "forty nine" → 49)."""
    if not text:
        return None
...
def normalize_quantity(text: str) -> int | None:
    ...
    Examples: ``"2"`` → 2, ``"two"`` → 2, ``"2 pcs"`` → 2,
    ``"half dozen"`` → 6, ``"one dozen"`` → 12.
    """
    if not text:
        return None
...
def extract_weight_quantity(text: str) -> dict | None:
    """Parse weight-style quantities like ``"half pound"``, ``"6 oz"``.

    Returns ``{"value": float, "unit": "oz"|"lb", "ounces": float}`` or
    ``None`` if the text contains no weight phrase.
    """
    if not text:
        return None
...
def detect_quantity(text: str) -> dict | None:
    """Detect and classify a quantity expression.

    Returns ``{"type": "exact"|"incremental"|"vague", "value": int | None}``.
    """
    normalized = (text or "").lower().strip()
    if not normalized:
        return None
    ...
    if value is not None:
        ...
        return {"type": quantity_type, "value": value}

    if any(re.search(pattern, normalized) for pattern in INCREMENTAL_PATTERNS):
        return {"type": "incremental", "value": 1}
